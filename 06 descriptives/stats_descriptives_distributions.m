...
x = linspace(-4,4,N);
gausdist = normpdf(x); % norm(normal)pdf(probability distribution function)
...
% is this a probability distribution? : No (sum != 1)
sum(gausdist)
% try scaling by dx...
sum(gausdist)*diff(x(1:2))
%%
figure(1), clf
subplot(211)
plot(x,gausdist/sum(gausdist),'linew',2)
title('Analytic Gaussian (normal) distribution')

% is this a probability distribution? : Yes (sum == 1)
sum(gausdist/sum(gausdist))
...
N = 1001;
x = linspace(0,10,N);
lognormdist = lognpdf(x); % lognormal
